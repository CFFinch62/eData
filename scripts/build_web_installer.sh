...
#
# Produces docs/firmware/eData-x3-<version>.bin (a single image containing
...
DATE="$(date +%Y-%m-%d)"
OUT="docs/firmware/eData-x3-${VERSION}.bin"

echo "==> Merging flash image  (version ${VERSION})"
mkdir -p docs/firmware
rm -f docs/firmware/eData-x3-*.bin   # keep exactly one build in the repo
  ...
  "builds": [
    {
      "chipFamily": "ESP32-C3",
      "parts": [
        { "path": "firmware/eData-x3-${VERSION}.bin", "offset": 0 }
      ]
    }
  ]
}
