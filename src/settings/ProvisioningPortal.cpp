...
constexpr char PAGE_PROFILE_FORM_TAIL[] =
    "<button type='submit' name='save' value='1'>Save</button> "
    "<button type='submit' name='use' value='1'>Save &amp; use now</button>"
    "</form>";

constexpr char PAGE_DANGER_AND_TAIL[] =
...
// 13 items (None..TimeToGo, see DataCatalog.h) + a trailing "</select>" fits
// comfortably in 900 bytes - each option is under 40 bytes.
void appendDataItemSelect(char* out, size_t outLen, const char* fieldName, DataItem current) {
  int n = std::snprintf(out, outLen, "<select name='%s'>", fieldName);
  for (int i = 0; i < DATA_ITEM_COUNT && n > 0 && static_cast<size_t>(n) < outLen; ++i) {
    const DataItem item = static_cast<DataItem>(i);
    n += std::snprintf(out + n, outLen - static_cast<size_t>(n), "<option value='%d'%s>%s</option>", i,
                        item == current ? " selected" : "", dataItemLabel(item));
  }
  if (n > 0 && static_cast<size_t>(n) < outLen) std::snprintf(out + n, outLen - static_cast<size_t>(n), "</select>");
}

// Box count decides how the 8 slots below are grouped into pages on the
// device (1 box -> 8 pages, 2 -> 4 pages, 4 -> 2 pages), skipping any slot
// left "Unused" - see PageLayout::buildPages. This does not reboot the
// device when saved (see handleSaveDisplay); it can't affect the network.
void sendDisplayLayoutSection(const DisplayConfig& display) {
  char buf[900];
  server.sendContent("<h3>Display layout</h3><form method='POST' action='/save-display'>"
                     "<label>Boxes per screen</label><select name='boxcount'>");
  for (int n : {1, 2, 4}) {
    std::snprintf(buf, sizeof(buf), "<option value='%d'%s>%d</option>", n, display.boxCount == n ? " selected" : "",
                  n);
    server.sendContent(buf);
  }
  server.sendContent("</select><div class='hint'>1 box: full-screen, one item at a time. 2: stacked top/bottom. "
                     "4: a 2&times;2 grid. Unused slots below are skipped, so fewer configured items means "
                     "fewer pages to scroll through, not blank ones.</div>");

  for (int i = 0; i < 8; ++i) {
    char label[16];
    char field[8];
    std::snprintf(label, sizeof(label), "Slot %d", i + 1);
    std::snprintf(field, sizeof(field), "slot%d", i);
    char select[900];
    appendDataItemSelect(select, sizeof(select), field, display.slots[i]);
    std::snprintf(buf, sizeof(buf), "<label>%s</label>%s", label, select);
    server.sendContent(buf);
  }
  server.sendContent("<button type='submit'>Save layout</button></form>");
}
...
void sendForm() {
  ...
  std::snprintf(buf, sizeof(buf),
                ...
                "<input type='hidden' name='slot' value='%d'>"
                "<label>Name</label><input name='name' maxlength='16' value='%s' placeholder='e.g. Helm multiplexer'>"
  ...
  server.sendContent(PAGE_PROFILE_FORM_TAIL);
  sendDisplayLayoutSection(cfg.display);
  server.sendContent(PAGE_DANGER_AND_TAIL);
  server.sendContent("");
}
...
// Unlike handleSave/handleUse, this never reboots: box count and slot
// assignment have no effect on Wi-Fi or the NMEA source, so main.cpp picks
// up the change on its next loop() tick via takeDisplayConfigChanged().
void handleSaveDisplay() {
  AppSettings cfg;
  loadAppSettings(cfg);

  const long boxCount = server.arg("boxcount").toInt();
  cfg.display.boxCount = (boxCount == 2 || boxCount == 4) ? static_cast<uint8_t>(boxCount) : 1;

  for (int i = 0; i < 8; ++i) {
    char field[8];
    std::snprintf(field, sizeof(field), "slot%d", i);
    const long v = server.arg(field).toInt();
    cfg.display.slots[i] = (v >= 0 && v < DATA_ITEM_COUNT) ? static_cast<DataItem>(v) : DataItem::None;
  }

  saveAppSettings(cfg);
  markDisplayConfigChanged();
  Serial.printf(LOG_TAG "Display layout saved: %u box(es) per screen\n", cfg.display.boxCount);
  server.sendHeader("Location", "/");
  server.send(303);
}
